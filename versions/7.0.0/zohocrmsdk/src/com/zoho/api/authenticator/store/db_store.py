...
class DBStore(TokenStore):
    ...
    def __init__(self, host=Constants.MYSQL_HOST, database_name=Constants.MYSQL_DATABASE_NAME,
        ...
        if not re.match(r'^[A-Za-z0-9_]+$', table_name):
            raise ValueError("Invalid table name: must contain only alphanumeric characters and underscores")
        self.__host = host
        ...
        # Backtick-quoted identifier safe for SQL. Regex above guarantees no backticks in the name.
        self.__table_name_quoted = "`" + table_name + "`"
    ...
    def find_token(self, token):
        try:
            ...
            raise ImportError(Constants.MYSQL_IMPORT_ERROR)
        cursor = None
        connection = None
        try:
            ...
            if isinstance(token, OAuthToken):
                ...
                if oauth_token.get_user_signature() is not None:
                    ...
                    query= query + " where access_token=%s"
                    param = oauth_token.get_access_token()
                    ...
                    if oauth_token.get_grant_token() is not None and len(oauth_token.get_grant_token()) > 0:
                        query = query + " where grant_token=%s"
                        param = oauth_token.get_grant_token()
                    elif oauth_token.get_refresh_token() is not None and len(oauth_token.get_refresh_token()) > 0:
                        query = query + " where refresh_token=%s"
                        param = oauth_token.get_refresh_token()
                query = query + " limit 1"
    ...
    def save_token(self, token):
        ...
        try:
            connection = mysql.connector.connect(host=self.__host, database=self.__database_name, user=self.__user_name,password=self.__password, port=self.__port_number)
            oauthToken = token
            query = "update " + self.__table_name_quoted + " set "
            set_params = []
            where_param = None
            if oauthToken.get_user_signature() is not None:
                name = oauthToken.get_user_signature().get_name()
                if name is not None and len(name) > 0:
                    set_clause, set_params = self.set_token(oauthToken)
                    query = query + set_clause + " where user_name=%s"
                    where_param = name
            elif oauthToken.get_access_token() is not None and len(oauthToken.get_access_token()) > 0 and self.are_all_objects_null([oauthToken.get_client_id(), oauthToken.get_client_secret()]):
                set_clause, set_params = self.set_token(oauthToken)
                query = query + set_clause + " where access_token=%s"
                where_param = oauthToken.get_access_token()
            elif ((oauthToken.get_refresh_token() is not None and len(oauthToken.get_refresh_token()) > 0) or (oauthToken.get_grant_token() is not None and len(oauthToken.get_grant_token()) > 0)) and oauthToken.get_client_id() is not None and oauthToken.get_client_secret() is not None:
                if oauthToken.get_grant_token() is not None and len(oauthToken.get_grant_token()) > 0:
                    set_clause, set_params = self.set_token(oauthToken)
                    query = query + set_clause + " where grant_token=%s"
                    where_param = oauthToken.get_grant_token()
                elif oauthToken.get_refresh_token() is not None and len(oauthToken.get_refresh_token()) > 0:
                    set_clause, set_params = self.set_token(oauthToken)
                    query = query + set_clause + " where refresh_token=%s"
                    where_param = oauthToken.get_refresh_token()
            query = query + " limit 1"
            try:
                cursor = connection.cursor()
                ...
                if cursor.rowcount <= 0:
                    ...
                    query = "insert into " + self.__table_name_quoted + " (id,user_name,client_id,client_secret,refresh_token,access_token,grant_token,expiry_time,redirect_url,api_domain) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);"
    ...
    def set_token(self, oauth_token):
        ...
        if oauth_token.get_user_signature() is not None:
            name = oauth_token.get_user_signature().get_name()
            if name is not None and len(name) > 0:
                query.append(Constants.USER_NAME + "=%s")
                params.append(name)
        if oauth_token.get_api_domain() is not None:
            query.append(Constants.API_DOMAIN + "=%s")
            params.append(oauth_token.get_api_domain())
        if oauth_token.get_client_id() is not None:
            query.append(Constants.CLIENT_ID + "=%s")
            params.append(oauth_token.get_client_id())
        if oauth_token.get_client_secret() is not None:
            query.append(Constants.CLIENT_SECRET + "=%s")
            params.append(oauth_token.get_client_secret())
        if oauth_token.get_refresh_token() is not None:
            query.append(Constants.REFRESH_TOKEN + "=%s")
            params.append(oauth_token.get_refresh_token())
        if oauth_token.get_access_token() is not None:
            query.append(Constants.ACCESS_TOKEN + "=%s")
            params.append(oauth_token.get_access_token())
        if oauth_token.get_grant_token() is not None:
            query.append(Constants.GRANT_TOKEN + "=%s")
            params.append(oauth_token.get_grant_token())
        if oauth_token.get_expires_in() is not None:
            query.append(Constants.EXPIRY_TIME + "=%s")
            params.append(oauth_token.get_expires_in())
        if oauth_token.get_redirect_url() is not None:
            query.append(Constants.REDIRECT_URL + "=%s")
            params.append(oauth_token.get_redirect_url())
        return str.join(",", query), params
    ...
    def delete_token(self, id):
        ...
        try:
            ...
            try:
                cursor = connection.cursor()
                query = "delete from " + self.__table_name_quoted + " where id = %s;"
    ...
    def delete_tokens(self):
        ...
        try:
            ...
            try:
                cursor = connection.cursor()
                query = 'delete from ' + self.__table_name_quoted + ";"
                cursor.execute(query)
                connection.commit()
    ...
    def find_token_by_id(self, id):
        ...
        try:
            connection = mysql.connector.connect(host=self.__host, database=self.__database_name, user=self.__user_name, password=self.__password, port=self.__port_number)
            try:
                query = "select * from " + self.__table_name_quoted + " where id = %s"
    ...
    def generate_id(self):
        ...
        try:
            connection = mysql.connector.connect(host=self.__host, database=self.__database_name, user=self.__user_name, password=self.__password, port=self.__port_number)
            query = "select  max(id) as id from " + self.__table_name_quoted + ";"
            cursor = connection.cursor()
            cursor.execute(query)
            result = cursor.fetchone()
            max1 = result[0]
