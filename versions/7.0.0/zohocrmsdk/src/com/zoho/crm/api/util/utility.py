...
class Utility(object):
    ...
    def check_data_type(value, data_type):
        ...
        if data_type in Constants.DATA_TYPE:
            ...
            class_name = Utility.get_java_like_canonical_name(value)
            if class_name.lower() == data_type.lower():
                return True
        return False

    @staticmethod
    def get_java_like_canonical_name(obj):
        cls = obj.__class__
        module_parts = cls.__module__.split('.')
        package = '.'.join(module_parts[:-1])  # drop file name
    ...
    def get_user_name(token):
        ...
        user_name = None
        param_instance = ParameterMap()
        param_instance.add(GetUsersParam.type, Constants.CURRENTUSER)
