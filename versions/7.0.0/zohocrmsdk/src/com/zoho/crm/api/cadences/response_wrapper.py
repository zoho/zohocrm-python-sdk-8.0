...
class ResponseWrapper(ResponseHandler):
	...
	def get_cadences(self):
		...
		return self.__cadences

	def set_cadences(self, cadences):
		"""
		The method to set the value to cadences

		Parameters:
			cadences (list) : An instance of list
		"""

		if cadences is not None and not isinstance(cadences, list):
			raise SDKException(Constants.DATA_TYPE_ERROR, 'KEY: cadences EXPECTED TYPE: list', None, None)
		
		self.__cadences = cadences
		self.__key_modified['cadences'] = 1
	...
	def set_info(self, info):
		...
		try:
			from zohocrmsdk.src.com.zoho.crm.api.cadences.info import Info
		except Exception:
