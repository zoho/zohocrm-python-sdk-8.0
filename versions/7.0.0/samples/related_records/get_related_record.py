...
from zohocrmsdk.src.com.zoho.crm.api.record import FileDetails, Reminder, Participants, PricingDetails, Record, \
    LineTax, Comment, ImageUpload, RemindAt, RecurringActivity, Consent
from zohocrmsdk.src.com.zoho.crm.api.related_records import RelatedRecordsOperations, GetRelatedRecordHeader, \
    ResponseWrapper, FileBodyWrapper, APIException, GetRelatedRecordParam
...
class GetRelatedRecord:
    ...
    def get_related_record(module_api_name, record_id, related_list_api_name, related_list_id, destination_folder):
        ...
        param_instance.add(GetRelatedRecordParam.fields, "id")
        header_instance = HeaderMap()
        header_instance.add(GetRelatedRecordHeader.if_modified_since,
                            datetime.fromisoformat('2019-10-15T05:00:00+05:30'))
        # Call get_related_record method that takes related_list_id, record_id and header_instance as parameter
        response = related_records_operations.get_related_record(related_list_id, record_id, param_instance,
                                                                 header_instance)
                            ...
                            if isinstance(value, list):
                                if len(value) > 0:
                                    if isinstance(value[0], FileDetails):
                                        ...
                                        tax = value[0]
                                        print("Record Tax Name: " + tax.get_name())
                                        ...
                                        for attachment in attachments:
                                            ...
                                            if parent_id is not None:
                                                print(
                                                    "Record Attachment parent record Name: " + parent_id.get_key_value(
                                                        "name"))
...
GetRelatedRecord.get_related_record(module_api_name="Leads", record_id=4402480774074, related_list_api_name="Notes",
                                    related_list_id=440248001495003, destination_folder="/Users/file")
