...
class CustomStoreSQLite(TokenStore):
    """
    ...
    def set_merge_data(oauth_token, next_record):
        if oauth_token.get_id() is None:
            oauth_token.set_id(CustomStoreSQLite.get_data(next_record[0]))
        if oauth_token.get_user_signature() is None:
            name = CustomStoreSQLite.get_data(next_record[1])
            if name is not None:
                oauth_token.set_user_signature(UserSignature(name))
        if oauth_token.get_client_id() is None:
            oauth_token.set_client_id(CustomStoreSQLite.get_data(next_record[2]))
        if oauth_token.get_client_secret() is None:
            oauth_token.set_client_secret(CustomStoreSQLite.get_data(next_record[3]))
        if oauth_token.get_refresh_token() is None:
            oauth_token.set_refresh_token(CustomStoreSQLite.get_data(next_record[4]))
        if oauth_token.get_access_token() is None:
            oauth_token.set_access_token(CustomStoreSQLite.get_data(next_record[5]))
        if oauth_token.get_grant_token() is None:
            oauth_token.set_grant_token(CustomStoreSQLite.get_data(next_record[6]))
        if oauth_token.get_expires_in() is None:
            expiresIn = CustomStoreSQLite.get_data(next_record[7])
            if expiresIn is not None:
                oauth_token.set_expires_in(str(expiresIn))
        if oauth_token.get_redirect_url() is None:
            oauth_token.set_redirect_url(CustomStoreSQLite.get_data(next_record[8]))
        if oauth_token.get_api_domain() is None:
            oauth_token.set_api_domain(CustomStoreSQLite.get_data(next_record[9]))
