...
class EnableNotifications:
    ...
    def enable_notifications():
        try:
            ...
            notification1.set_events(['Deals.all'])
            notification1.set_channel_expiry(datetime(2025, 12, 31, 23, 59, 59))
            notification1.set_token("TOKEN_FOR_VERIFICATION_OF_100000006800211")
            notification1.set_notify_url("https://www.example.com/webhook1")
            notifications.append(notification1)
            
            notification2 = Notification()
            notification2.set_channel_id("1000000068002")
            notification2.set_events(['Accounts.all'])
            notification2.set_channel_expiry(datetime(2025, 12, 31, 23, 59, 59))
            notification2.set_token("TOKEN_FOR_VERIFICATION_OF_1000000068002")
            notification2.set_notify_url("https://www.example.com/webhook2")
            notifications.append(notification2)
            
            body_wrapper.set_watch(notifications)
            
            response = notifications_operations.enable_notifications(body_wrapper)
            ...
            print("Exception when calling enable_notifications: " + str(e))
