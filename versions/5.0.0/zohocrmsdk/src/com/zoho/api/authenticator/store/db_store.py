...
class DBStore(TokenStore):
    ...
    def save_token(self, token):
        ...
        try:
            ...
            if oauthToken.get_user_signature() is not None:
                ...
                if name is not None and len(name) > 0:
                    query = query + self.set_token(oauthToken) + " where user_name=%s"
                    param = name
                ...
                query = query + self.set_token(oauthToken) + " where access_token=%s"
                param = oauthToken.get_access_token()
                ...
                if oauthToken.get_grant_token() is not None and len(oauthToken.get_grant_token()) > 0:
                    query = query + self.set_token(oauthToken) + " where grant_token=%s"
                    param = oauthToken.get_grant_token()
                elif oauthToken.get_refresh_token() is not None and len(oauthToken.get_refresh_token()) > 0:
                    query = query + self.set_token(oauthToken) + " where refresh_token=%s"
                    param = oauthToken.get_refresh_token()
            query = query + " limit 1"
            try:
                cursor = connection.cursor()
                cursor.execute(query, (param,))
    ...
    def delete_token(self, id):
        ...
        try:
            ...
            try:
                cursor = connection.cursor()
                query = "delete from " + self.__table_name + " where id = %s;"
    ...
    def find_token_by_id(self, id):
        ...
        try:
            connection = mysql.connector.connect(host=self.__host, database=self.__database_name, user=self.__user_name, password=self.__password, port=self.__port_number)
            try:
                query = "select * from " + self.__table_name + " where id = %s"
